local lexer = require("vortex/lexer")
local util  = require("vortex/util")
...
local parse_table = function(ls)
    ...
    return Table_Expr(ls, table.unpack(tbl))
end
...
local parse_list = function(ls)
    ...
    assert_tok(ls, "]")
    ls:get()
    return List_Expr(ls, table.unpack(el))
end
...
local parse_function = function(ls, obj, block)
    ...
    if n == "|" or n == "case" then
        ...
        fnexpr = Function_Expr(ls, ids, defs,
            Match_Expr(ls, el, table.unpack(parse_match_body(ls))))
    else
...
local parse_enum = function(ls)
    ...
    return Enum_Expr(ls, table.unpack(t))
end
...
local parse_table_pattern = function(ls, let)
    ...
    return Table_Pattern(ls, table.unpack(tbl))
end
...
local parse_primarypattern = function(ls, let)
    ...
    elseif tn == "$" or tn == "$(" and not let then
        ...
        if tok.name == "(" then
            ...
            assert_tok(ls, ")")
            ls:get()
            return Object_Pattern(ls, exp, table.unpack(tbl))
        end
    ...
    elseif tn == "<ident>" then
        ...
        elseif tok.name == "(" then
            ...
            assert_tok(ls, ")")
            ls:get()
            return Object_Pattern(ls, Symbol_Expr(ls, v), table.unpack(tbl))
        else
            ls.ndstack:pop()
...
local parse_match = function(ls, block)
    ...
    assert_tok(ls, "->")
    ls:get()
    return Match_Expr(ls, el, table.unpack(parse_match_body(ls, block)))
end
...
parse_do = function(ls, ed)
    ...
    if not (ed and tok.name == ed) then
        assert_tok(ls, ";;", "end")
        ls:get()
        return Do_Expr(ls, table.unpack(exprs)), false
    end
    return Do_Expr(ls, table.unpack(exprs)), true
end
...
local parse_result = function(ls)
    ...
    if ls.token.name == "(" then
        ...
        assert_tok(ls, ")")
        ls:get()
        return Result_Expr(ls, table.unpack(exprs))
    end
...
local parse_return = function(ls)
    ...
    if ls.token.name == "(" then
        ...
        assert_tok(ls, ")")
        ls:get()
        return Return_Expr(ls, table.unpack(exprs))
    end
...
local parse_yield = function(ls)
    ...
    if ls.token.name == "(" then
        ...
        assert_tok(ls, ")")
        ls:get()
        return Yield_Expr(ls, table.unpack(exprs))
    end
...
local parse_new = function(ls)
    ...
    assert_tok(ls, ")")
    ls:get()
    return New_Expr(ls, pex, table.unpack(el))
end
...
parse_primaryexpr = function(ls)
    ...
    elseif tn == "<ident>" then
        ...
        if tok.name == "!" then
            ...
            assert_tok(ls, ")")
            ls:get()
            return macro_expand(ls, v, table.unpack(lst))
        else
            return Symbol_Expr(ls, v)
        end
    ...
    elseif tn == "<begstring>" then
        ...
        if #exprs > 1 then
            ...
            return Call_Expr(ls, false, Symbol_Expr(nil,
                lazy_rt_fun("str_fmt")), table.unpack(exprs))
        end
    ...
    while true do
        if tok.name == "." or tok.name == ":" then
            ...
            if mcall then
                ...
                exp = Call_Expr(ls, exp, Value_Expr(ls, s), table.unpack(el))
            else
        ...
        elseif tok.name == "(" then
            ...
            exp = Call_Expr(ls, false, exp, table.unpack(el))
        else
            return exp
        end
    end
...
build = function(ast, nort)
    ...
    local hdr = {
        ...
        gen_local('_ENV', rts..'.__vx_def_env'),
        (not nort) and gen_require("vxrt/init") or nil
    }
    local rtcache = {}
    get_rt_fun = function(name)
        local n = rtcache[name]
